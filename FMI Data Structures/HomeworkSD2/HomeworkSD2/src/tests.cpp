...
TEST_CASE("Create and save hierarchy") {
    SECTION("Errors") {
        ...
        const string error_format_example2 = "Uspeshnia--Gosho\n";
        const string error_format_example3 = "-Uspeshnia-Gosho\n";

        const string no_parent = "Uspeshnia   -  MishoPetrov \n Ivan-Dragan";

        REQUIRE_THROWS(create(error_format));
        REQUIRE_THROWS(create(error_format_example2));
        REQUIRE_THROWS(create(error_format_example3));
...
TEST_CASE("Join") {
    ...
    const string should_not_be_created1 =
        "Uspeshnia-A\n"
        "A-B\n";

    const string should_not_be_created2 =
        "Uspeshnia-B\n"
        "B-A\n";

    Hierarchy j1(should_not_be_created1);
    Hierarchy j2(should_not_be_created2);

    Hierarchy joined4 = j1.join(j2);
    REQUIRE(joined4.num_employees() == 0);
    REQUIRE(joined4.print() == "");
    ...
    const string exampleFromDiscord1 =
        "Uspeshnia-A\n"
        "A-B\n"
        "A-C\n"
        "C-D\n"
    ...
    const string exampleFromDiscord2 =
        "Uspeshnia-A\n"
        "A-B\n"
        "A-C\n"
        "A-D\n"
    ...
    Hierarchy ex1(exampleFromDiscord1);
    Hierarchy ex2(exampleFromDiscord2);

    Hierarchy joined3 = ex1.join(ex2);
    REQUIRE(joined3.longest_chain() == 4);
    REQUIRE(joined3.num_employees() == 7);
    REQUIRE(joined3.manager("F") == "B");
    REQUIRE(joined3.manager("D") == "A");
    REQUIRE(joined3.manager("Q") == "D");
    REQUIRE(joined3.getSalary("C") == 0);
    REQUIRE(joined3.num_employees() == 7);
    REQUIRE(joined3.print() == "Uspeshnia-A\nA-B\nA-C\nA-D\nB-F\nD-Q\n");
}
...
TEST_CASE("Fire") {
    ...
    REQUIRE(h.num_subordinates("Gosho") == 2);
    REQUIRE(h.fire("Gosho"));
    REQUIRE(h.num_subordinates(TheBoss) == 4);
    CHECK(!h.find("Gosho"));

    CHECK(h.find("Slav2"));
    REQUIRE(h.num_subordinates("Slavi") == 2);
    REQUIRE(h.fire("Slav2"));
    REQUIRE(h.num_subordinates("Slavi") == 1);

    REQUIRE(h.longest_chain() == 5);
    CHECK(h.find("Mecho"));
    REQUIRE(h.fire("Mecho"));
    REQUIRE(h.num_subordinates("Slav1") == 1);
    REQUIRE(h.longest_chain() == 4);
    REQUIRE(!h.fire("Mecho"));
    REQUIRE(h.fire("Slav1"));
    REQUIRE(h.num_subordinates("Slavi") == 1);
    REQUIRE(h.longest_chain() == 3);
}
